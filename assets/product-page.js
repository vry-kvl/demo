(() => {
  // app/scripts/common/sections/product-recommendations.js
  ...
  // app/scripts/common/function/product-form.js
  var productForm = class extends HTMLElement {
    ...
    onChange(event) {
      ...
      if (!target.classList.contains("js-variant-change"))
        return;
    ...
    linkOptions() {
      ...
      if (!Array.from(document.querySelectorAll("select[name*=option], input[name*=option]:checked")).filter((i2) => i2 != null).length)
        return;
    ...
    updateOptionsInSelector(index) {
      ...
      switch (index) {
        case 1:
          key = "root";
          selector3 = optionEl.option1;
          break;
        case 2:
          key = optionEl.option1?.value;
          selector3 = optionEl.option2;
          break;
        default:
          key = optionEl.option1?.value;
          selector3 = optionEl.option3;
          key += " / " + optionEl.option2?.value;
          index = 3;
      }
      if (!selector3) {
        selector3 = Array.from(document.querySelectorAll(`select[name=option${index}], input[name=option${index}]`)).find((i) => i != null);
      }
      ;
      const availableOptions = this.optionsMap[key] || [];
      const optionsValid = this.availableOptions[key] || [];
      const initialValue = selector3?.value || "null";
      ;
      if (!selector3)
        return;
      if (selector3.tagName == "SELECT") {
        Array.from(selector3.querySelectorAll("option")).forEach((option) => {
        ...
        if (availableOptions.length && !availableOptions.includes(initialValue)) {
          selector3.value = availableOptions[0];
        }
      } else {
        Array.from(this.querySelectorAll(`[name=${selector3.name}]`)).forEach((input) => {
        ...
        if (availableOptions.length && !availableOptions.includes(initialValue)) {
          ...
          const findAvailableInput = Array.from(this.querySelectorAll(`[name=${selector3.name}]`)).find((input) => input.value == tempInput.value);
  ...
  */
  ...
  var fly_to_cart_default = flyToCart = function(node, placementNode) {
    ...
    const miniCart = Array.from(document.querySelectorAll(".js-mini-cart")).find((item) => item.getBoundingClientRect().width > 0);
    if (!miniCart)
      return;
  ...
  // app/scripts/common/function/product-bundle.js
  var productBundle = class extends HTMLElement {
    constructor() {
      super();
      this.Cart = new Cart();
      this.addEventListener("change", this.onChange.bind(this));
    ...
    submit() {
      const items = Array.from(this.querySelectorAll('[name="bundleItem"]:checked')).map((i) => {
        return { id: i.value };
      });
  ...
  // app/scripts/common/function/product-slider.js
  var productSliderComponent = class extends HTMLElement {
    constructor() {
      super();
      this.cache = {};
      try {
    ...
    initZoom() {
      ...
      if (this.dataset.type != "main" || !zoomGalleryJSON)
        return;
    ...
    onResize() {
      if (this.mobileDevice == this.isMobileDevice())
        return;
      this.unLoad();
      setTimeout(() => {
        this.load();
    ...
    loadYTVideo() {
      ...
      window["onPlayerReady"] = (event) => {
        let video = event.target;
        if (!video.h.closest('[data-media-index="1"]'))
          return;
        video.playVideo();
  ...
  // app/scripts/common/function/store-availability.js
  var storeAvailability = class extends HTMLElement {
    constructor() {
      super();
  ...
  // app/scripts/common/sections/product.js
  ...
  var productDescription = class extends HTMLElement {
    constructor() {
      super();
    ...
    toggleTab() {
      ...
      window.addEventListener("resize", (e) => {
        if (!window.matchMedia("(min-width: 750px)").matches || this.querySelector(".active"))
          return;
  ...
  register("product-page", product_default);
  load("*");
})();
